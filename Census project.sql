...
select state,round(avg(growth),0) avg_growth from dataset1 group by state;
...
select state,round(avg(growth),0) avg_growth from dataset1 group by state order by avg_growth desc limit 3;
...
(select a.district,a.state,a.growth ,b.population from dataset1 a inner join dataset2 b on a.district=b.district) d) e
group by e.state)m;
